...
def atividadeUsuario():
    ...
    while not digitouAtividadeDireito:
        ...
        print('[2] De 3 à 5 dias por semana')
...
def metaCaloria(caloriaDesejada,caloriaDigitada):
    meta = caloriaDesejada - caloriaDigitada
    meta = round(meta,2)
    if meta == 0:
        print('\nParabéns, você atingiu a quantidade ideal')
    elif meta > 1:
        print('\nVocê ingeriu {} a menos do que deveria.'.format(meta))
    else:
        print('Você ingeriu {} calorias a menos do que deveria.'.format(meta))
    return meta
...
'''MUDANÇAS A APLICAR:
-COLHER E VALIDAR DIGITAÇÃO DE PESO E ALTURA'''
